...
def generate_daily_pngs(data_folder, output_folder):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, f'avg_power_{date_str}.png'), dpi=300)
    plt.close()
    ...
    plt.savefig(os.path.join(output_folder, f'spectrogram_{date_str}.png'), dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    data_folder = filedialog.askdirectory(title="Select the day's data folder")
    if not data_folder:
    ...
    generate_daily_pngs(data_folder, output_folder)
